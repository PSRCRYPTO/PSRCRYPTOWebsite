...
// Ryan's BASIC Environment
var classBASIC = function ( OS )  {
    this.OS = OS;
};

classBASIC.prototype.getEnv = function() {
  var hOS = this.OS;
  return {
      tty: {
          ...
          writeChar: function(ch) { hOS.output.pushBuffer(ch.replace(/\r/g, '\n')); },
          writeString: function(string) { hOS.output.pushBuffer(string.replace(/\r/g, '\n')); },
          readChar: function(callback) {
            callback(console.getc());
          },
          readLine: function(callback, prompt) {
            hOS.output.pushBuffer(prompt);
...
classBASIC.prototype.run = function( source, callback ) {
    try {
        program = basic.compile(source);
    } catch (pe) {
        if (pe instanceof basic.ParseError) {
            this.OS.output.pushBuffer('\n' + pe.message + ' (source line: ' + pe.line + ', column: ' + pe.column + ')\n');
            callback();
            return;
        } else {
            callback();
